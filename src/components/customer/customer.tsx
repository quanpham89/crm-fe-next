...
const Customer = (props: any) => {
  const { user } = props;
  const [isOpenModalConfirmHidden, setIsOpenModalConfirmHidden] =
    useState(false);
  ...
  const getDataUser = async () => {
    const response = await handleGetDataUserCustomer(
  ...
  const items = [
    ...
    },
    {
      ...
      children:
        currentCustomerUser?.user?.role === "BUSINESSMAN"
          ? "Người kinh doanh"
          : "Khách hàng",
    },
    {
      key: "5",
      label: "Trạng thái",
      children: currentCustomerUser?.user?.isActive
        ? "Hoạt động"
        : "vô hiệu hóa",
    },
    {
  ...
  return (
    <>
      ...
      >
        <Button onClick={() => setIsOpenModalConfirmHidden(true)}>
          Vô hiêu hóa tài khoản
        </Button>
        <Button onClick={() => setIsOpenUpdateCustomer(true)}>
          Cập nhập thông tin
        </Button>
      </div>
      <ModalConfirmHidden
